...
(() => {
  const report = async (payload) => {
    try {
      ...
      await vscode.postMessage({ type: "__mindflowRuntimeIssue", payload });
    } catch {
      // ignore
    }
  };
...
function injectBaseLayout(html) {
  const style = `<style data-mindflow-webview-base>
html, body {
  height: 100%;
  width: 100%;
  margin: 0;
...
async function main() {
  ...
  const server = http.createServer(
    withErrorBoundary(async (req, res) => {
      ...
      if (u.pathname === "/mindflow/state" && req.method === "POST") {
        const body = await readJson(req);

        if (typeof body?.theme === "string") {
          const theme = body.theme === "light" ? "light" : "dark";
          state._mindflow?.setTheme?.(theme);
        }

        if (body?.activeDocument === null) {
          state._mindflow?.setActiveDocument?.(null);
          ...
          if (p) {
            state._mindflow?.setActiveDocument?.({ path: p, languageId, content });
          }
        }
      ...
      if (u.pathname === "/vscode/message" && req.method === "POST") {
        ...
        if (message?.type === "__mindflowRuntimeIssue" && message.payload && typeof message.payload === "object") {
...
function createVscodeApi(state, originForApi) {
  ...
  state._mindflow = state._mindflow ?? {};
  state._mindflow.setTheme = (theme) => {
  ...
  state._mindflow.setActiveDocument = (nextDoc) => {
  ...
  return {
    ...
    chat: {
      registerChatSessionItemProvider() {
        throw new Error("chat API not supported in mindflow host (yet)");
      },
    },
    window: {
    ...
    version: "mindflow-vscode-host-0.0.1",
