...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__mindflowTypesettingHarness, { timeout: 60_000 });
    ...
    if (fontPath) {
      ...
      await page.evaluate(
        async ({ payload, name }) => {
          await window.__mindflowTypesettingHarness.setFontBase64(payload, "SimHei", name);
        },
    ...
    await page.evaluate(
      async ({ payload, name }) => {
        await window.__mindflowTypesettingHarness.loadDocxBase64(payload, name);
      },
    ...
    while (Date.now() < readyDeadline) {
      const status = await page.evaluate(() => window.__mindflowTypesettingStatus ?? null);
    ...
    const pdfBase64 = await page.evaluate(async () => {
      return window.__mindflowTypesettingHarness.exportPdfBase64();
    });
    ...
    if (options.layoutPath) {
      const layoutJson = await page.evaluate(async () => {
        return window.__mindflowTypesettingHarness.exportLayoutJson();
      });
    ...
    if (options.irPath) {
      const irJson = await page.evaluate(async () => {
        return window.__mindflowTypesettingHarness.exportIrJson();
      });
