...
export default {
  ...
  overview: {
    ...
    openClawDesc: 'MindFlow 正在操作 OpenClaw 的同一批文件。当前识别到的产物文件数：{count}。',
  ...
  ai: {
    ...
    regenerate: '重新生成',
    errorRetry: '发生错误，请重试',
    welcomeAgent: '我是 MindFlow Agent，告诉我你想完成的任务。',
    startTask: '输入任务指令开始',
    needApproval: '需要审批',
    sendToQueue: '加入队列',
    ...
    agentInputPlaceholder: '我是 MindFlow，这个模式下我拥有许多强力装备，可以帮你处理各种事务！',
    chatInputPlaceholder: '我是 MindFlow，你有什么想和我聊聊的？我知无不言',
    debugPanel: '调试面板',
  ...
  ribbon: {
    ...
    starProject: '给 MindFlow 点个 Star',
  ...
  plugins: {
    ...
    title: '插件',
    intro: 'MindFlow 会优先从默认插件目录（安装位置旁）加载插件，并兼容工作区 .mindflow/plugins、用户插件目录和内置资源。',
  ...
  settingsModal: {
    ...
    openClawTitle: 'OpenClaw 工作区',
    openClawDesc: '把一个外部 OpenClaw 工作区挂到当前 MindFlow 工作区，并明确这是一套共享真实文件而不是导入副本。',
    openClawEnabled: '启用集成',
    openClawDisabledDesc: 'OpenClaw 集成当前已关闭。重新启用前，MindFlow 会把该工作区当作普通文件夹处理。',
    ...
    openClawMountedWorkspaceHint: '把一个外部 OpenClaw 工作区路径挂到当前 MindFlow 工作区上。',
    ...
    openClawBoundaryHint: '你在 MindFlow 里的修改会直接写回 OpenClaw 正在使用的同一批文件。这是附着工作区，不是导入副本。',
...
5) 输出简洁、分条，使用中文`,
  ...
  imageManager: {
    ...
    renameDescription: 'MindFlow 将重命名图片文件并重写所有受影响的 Markdown 引用。',
    moveDescription: 'MindFlow 将把图片文件移动到目标文件夹，并在一次操作中更新所有受影响的 Markdown 引用。',
    closeDialog: '关闭对话框',
    newFileName: '新文件名',
