...
const GLOBAL_KEY = "__mindflowDevPerf";
const BASELINE_STORAGE_KEY = "mindflow:perf:baseline:v1";
...
declare global {
  interface Window {
    __mindflowRecordRenderSample?: (sample: DevRenderSample) => void;
  }
}
...
export function bootstrapDevPerfMonitor(): void {
  ...
  const host = window as unknown as { __mindflowDevPerfInitialized?: boolean };
  if (host.__mindflowDevPerfInitialized) {
    return;
  }
  if (localStorage.getItem("mindflow_perf_disabled") === "1") {
    return;
  }
  host.__mindflowDevPerfInitialized = true;

  getSnapshot();
  window.__mindflowRecordRenderSample = recordDevRenderSample;
