...
describe('themePlugin', () => {
  ...
  it('loads valid user themes, prefixes ids, and exposes them through lookups', async () => {
    ...
    invokeMock.mockImplementation(async (command: string, args?: Record<string, unknown>) => {
      ...
      if (command === 'read_text_file' && args?.path === '/vault/.mindflow/themes/ocean.json') {
  ...
  it('ignores malformed and structurally invalid user theme files', async () => {
    ...
    invokeMock.mockImplementation(async (command: string, args?: Record<string, unknown>) => {
      ...
      if (command === 'read_text_file' && args?.path === '/vault/.mindflow/themes/broken.json') {
        return 'not json';
      }
      if (command === 'read_text_file' && args?.path === '/vault/.mindflow/themes/invalid.json') {
        return JSON.stringify({ id: 'bad', name: 'Bad' });
      }
      if (command === 'read_text_file' && args?.path === '/vault/.mindflow/themes/valid.json') {
  ...
  it('exports and imports themes without leaking export metadata', () => {
    ...
    expect(parsed._exportedFrom).toBe('MindFlow');
