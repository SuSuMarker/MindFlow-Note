...
export function useConversationManager() {
  ...
  }, [agentSessions.length, chatSessions.length, deleteAgentSession, deleteChatSession, deleteResearchSession, researchSessions.length]);
  ...
  }, [chatMode, createAgentSession, createChatSession, resetResearch]);
  ...
  }, [agentSessions.length, chatMode, chatSessions.length, currentSessionId, deleteAgentSession, deleteChatSession, deleteResearchSession, researchSessions.length]);

  const canDeleteSession = useCallback((id: string, type: SessionType): boolean => {
    void id;
    if (type === "agent") return agentSessions.length > 1;
    if (type === "research") return researchSessions.length > 1;
    return chatSessions.length > 1;
  }, [agentSessions.length, chatSessions.length, researchSessions.length]);

  const canDeleteCurrentSession = useMemo(() => {
    if (!currentSessionId) return false;
    if (chatMode === "agent") return agentSessions.length > 1;
    if (chatMode === "research") return researchSessions.length > 1;
    return chatSessions.length > 1;
  }, [agentSessions.length, chatMode, chatSessions.length, currentSessionId, researchSessions.length]);
