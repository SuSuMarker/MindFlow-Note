...
const createEditorTheme = (fontSize: number) =>
  EditorView.theme({
    "&": {
      ...
      height: "100%",
      "--mindflow-codeblock-bg": "hsl(var(--muted) / 0.58)",
      "--mindflow-codeblock-bg-hover": "hsl(var(--muted) / 0.74)",
      "--mindflow-codeblock-bg-source": "hsl(var(--muted) / 0.7)",
      "--mindflow-codeblock-border": "hsl(var(--border) / 0.65)",
      "--mindflow-codeblock-border-soft": "hsl(var(--border) / 0.35)",
      "--mindflow-codeblock-shadow":
    ...
    ".cm-codeblock-widget": {
      ...
      position: "relative",
      overflow: "hidden",
      backgroundColor: "var(--mindflow-codeblock-bg)",
      border: "1px solid var(--mindflow-codeblock-border)",
      borderRadius: "14px",
      boxShadow: "var(--mindflow-codeblock-shadow)",
      backdropFilter: "blur(6px)",
    },
    ".cm-mindflow-codeblock-open": {
      ...
      fontFamily: "'JetBrains Mono', monospace",
      border: "1px solid var(--mindflow-codeblock-border)",
      borderBottom: "1px solid var(--mindflow-codeblock-border-soft)",
      borderRadius: "14px 14px 0 0",
      boxShadow: "var(--mindflow-codeblock-shadow)",
      backdropFilter: "blur(6px)",
    },
    ".cm-mindflow-codeblock-content-line": {
      backgroundColor: "var(--mindflow-codeblock-bg)",
      ...
      boxShadow:
        "inset 1px 0 0 var(--mindflow-codeblock-border), inset -1px 0 0 var(--mindflow-codeblock-border)",
    },
    ".cm-mindflow-codeblock-close": {
      paddingLeft: "14px !important",
      paddingRight: "14px !important",
      ...
      color: "hsl(var(--muted-foreground) / 0.78)",
      fontFamily: "'JetBrains Mono', monospace",
      borderLeft: "1px solid var(--mindflow-codeblock-border)",
      borderRight: "1px solid var(--mindflow-codeblock-border)",
      borderBottom: "1px solid var(--mindflow-codeblock-border)",
    ...
    ".cm-mindflow-codeblock-copy-anchor": {
      position: "absolute",
      top: "8px",
      right: "10px",
      zIndex: "2",
    ...
    ".cm-codeblock-source": {
      backgroundColor: "var(--mindflow-codeblock-bg-source)",
      color: "hsl(var(--foreground) / 0.96)",
      fontFamily: "'JetBrains Mono', monospace",
      paddingLeft: "14px !important",
      paddingRight: "14px !important",
    ...
    ".cm-codeblock-content": {
      backgroundColor: "var(--mindflow-codeblock-bg) !important",
      fontFamily: "'JetBrains Mono', monospace !important",
      color: "hsl(var(--foreground) / 0.96)",
...
class LiveCodeBlockCopyButtonWidget extends WidgetType {
  ...
  toDOM() {
    ...
    anchor.className = "cm-mindflow-codeblock-copy-anchor";
...
function buildEditableCodeBlockDecorations(state: EditorState): DecorationSet {
  ...
  syntaxTree(state).iterate({
    enter: (node) => {
      ...
      decorations.push(
        Decoration.line({ class: "cm-mindflow-codeblock-open" }).range(
          openLine.from,
        ),
      );
      decorations.push(
      ...
      for (
        ...
        decorations.push(
          Decoration.line({ class: "cm-mindflow-codeblock-content-line" }).range(
            line.from,
          ),
        );
      }
      decorations.push(
        Decoration.line({ class: "cm-mindflow-codeblock-close" }).range(
          closeLine.from,
        ),
      );
...
function createSelectionTraceControl(
  ...
  if (runtimeWindow) {
    ...
    w.__mindflowEditorTrace = traceApi;
    w.__cmSelectionTraceSessionId = sessionId;
    w.__mindflowEditorTraceSessionId = sessionId;
  }
  ...
  return {
    ...
    destroy: (reason) => {
      ...
      if (runtimeWindow) {
        ...
        if (w.__mindflowEditorTrace?.sessionId === sessionId) {
          delete w.__mindflowEditorTrace;
          delete w.__mindflowEditorTraceSessionId;
        }
      }
    },
  };
...
>(function CodeMirrorEditor(
  ...
  const { editorFontSize, editorInteractionTraceEnabled } = useUIStore();

  const markTransitionTrace = useCallback(
    (type: string, payload: Record<string, unknown>) => {
      (window as any).__mindflowEditorTrace?.mark?.(type, payload);
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    console.log("[mindflow][codemirror-props]", {
      filePath: resolvedFilePath,
      contentLength: content.length,
      mode: effectiveMode,
    });
  }, [content, effectiveMode, resolvedFilePath]);
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (link?.getAttribute("href")?.startsWith("mindflow://pdf")) {
        e.preventDefault();
        e.stopPropagation();
        const parsed = parseMindFlowLink(link.getAttribute("href")!);
        if (parsed?.file) {
          reportOperationError({
            source: "CodeMirrorEditor.handleClick",
  ...
  useEffect(() => {
    const traceApi = (window as any).__mindflowEditorTrace;
    if (!traceApi) return;
