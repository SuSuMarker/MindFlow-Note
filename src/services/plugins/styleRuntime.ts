...
const STYLE_EVENT = "mindflow-plugin-style-runtime-updated";
...
const scopeCss = (css: string, scopeId: string) => {
  const anchor = `[data-mindflow-plugin-scope=\"${scopeId}\"]`;
...
class PluginStyleRuntime {
  ...
  registerStyle(pluginId: string, input: PluginStyleInput): () => void {
    ...
    style.setAttribute("data-mindflow-plugin-style", pluginId);
    style.setAttribute("data-mindflow-plugin-style-id", id);
    style.setAttribute("data-mindflow-plugin-style-layer", layer);
    if (entry.scopeId) {
      style.setAttribute("data-mindflow-plugin-scope", entry.scopeId);
    }
