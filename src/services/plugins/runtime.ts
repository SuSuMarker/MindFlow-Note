...
const HOST_APP_VERSION =
  typeof __MINDFLOW_APP_VERSION__ === "string" && __MINDFLOW_APP_VERSION__.trim().length > 0
    ? __MINDFLOW_APP_VERSION__
    : "0.0.0";
...
`
    ) as (api: MindFlowPluginApi, plugin: PluginInfo) => PluginSetupResult;
  ...
  private createApi(
    ...
    const registerPluginCommand = (
      ...
      window.dispatchEvent(new CustomEvent("mindflow-plugin-commands-updated"));
    ...
    const storageKey = (key: string) => `mindflow-plugin:${info.id}:${key}`;
    ...
    return {
      ...
      ui: {
        notify: (message: string) => {
          ...
          window.dispatchEvent(
            new CustomEvent("mindflow-plugin-notify", {
      ...
      editor: {
        ...
        registerDecoration: (className: string, css: string) => {
          ...
          style.setAttribute("data-mindflow-plugin-editor-decoration", info.id);
