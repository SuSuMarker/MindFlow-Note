...
describe('parseMindFlowLink', () => {
  it('should parse valid mindflow link', () => {
    const result = parseMindFlowLink('mindflow://pdf?file=test.pdf&page=5&id=ann-123');
  ...
  it('should handle encoded file path', () => {
    const result = parseMindFlowLink('mindflow://pdf?file=%2Fpath%2Fto%2Ffile.pdf&page=1');
    expect(result?.file).toBe('/path/to/file.pdf');
  });

  it('should return null for non-mindflow links', () => {
    expect(parseMindFlowLink('http://example.com')).toBeNull();
    expect(parseMindFlowLink('mindflow://other')).toBeNull();
  });

  it('should handle partial params', () => {
    const result = parseMindFlowLink('mindflow://pdf?page=3');
    expect(result).toEqual({
      file: undefined,
      page: 3,
      id: undefined,
