...
describe("CodexPanel", () => {
  ...
  it("shows actionable guidance when the Codex extension download fails", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "MindFlow 已找到 Codex 扩展，但无法下载 VSIX 安装包。请检查网络或代理设置，或者手动导入 VSIX。",
        ),
      ).toBeTruthy();
    });
  ...
  it("shows actionable guidance when the downloaded VSIX cannot be extracted", async () => {
    invokeMock.mockImplementation((cmd: string) => {
      if (cmd === "codex_extension_get_status") {
        return Promise.resolve({
          installed: false,
      ...
      if (cmd === "codex_extension_install_latest") {
        return Promise.reject(new Error("Invalid path: Failed to extract VSIX (tar/unzip/Expand-Archive)"));
      }
      return Promise.resolve(null);
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "MindFlow 已下载 Codex 扩展，但无法在本地解压 VSIX 安装包。你可以手动导入 VSIX，或安装本地 unzip/tar 工具后重试。",
        ),
      ).toBeTruthy();
    });
  });
  ...
  it("shows actionable guidance when the built-in Codex runtime cannot start", async () => {
    invokeMock.mockImplementation((cmd: string) => {
      ...
      if (cmd === "codex_vscode_host_start") {
        return Promise.reject(
          new Error("Invalid path: Node runtime not found. Bundle node with the app or set MINDFLOW_NODE_PATH."),
        );
      }
      return Promise.resolve(null);
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "MindFlow 无法启动内置的 Codex 运行时。你可以稍后重试；如果持续失败，再考虑更新应用。",
        ),
      ).toBeTruthy();
    });
  });
