...
function formatCodexInstallError(message: string, normalized: string): string {
  if (
    normalized.includes("marketplace extensionquery failed") ||
    normalized.includes("marketplace response missing") ||
    normalized.includes("marketplace") && normalized.includes("timed out")
  ) {
    return "MindFlow 无法连接到 VS Code Marketplace 来查询最新的 Codex 扩展。请检查网络或代理设置，或者手动导入 VSIX。";
  }
  ...
  if (
    ...
    return "MindFlow 已找到 Codex 扩展，但无法下载 VSIX 安装包。请检查网络或代理设置，或者手动导入 VSIX。";
  }

  if (
    normalized.includes("failed to extract vsix") ||
    normalized.includes("vsix extract task failed") ||
    normalized.includes("extension/package.json not found")
  ) {
    return "MindFlow 已下载 Codex 扩展，但无法在本地解压 VSIX 安装包。你可以手动导入 VSIX，或安装本地 unzip/tar 工具后重试。";
  }

  if (
    normalized.includes("vsix is not openai.chatgpt") ||
    normalized.includes("vsix version mismatch") ||
    normalized.includes("vsix not found")
  ) {
    return "The selected VSIX isn't a valid Codex extension package. Choose the OpenAI Codex VSIX and try again.";
  }

  if (normalized.includes("timed out") && normalized.includes("connection")) {
    return "MindFlow 下载 Codex 扩展时已超时。请检查网络或代理设置，或者手动导入 VSIX。";
  }

  if (normalized.includes("connection")) {
    return "MindFlow 无法连接并下载 Codex 扩展。请检查网络或代理设置，或者手动导入 VSIX。";
  }

  return message;
}

function buildCodexErrorDisplay(action: string, rawError: unknown): CodexErrorDisplay {
  const detail = normalizeErrorMessage(rawError);
  return {
    message: formatCodexUserError(action, rawError),
    detail,
  };
}
...
export function formatCodexUserError(action: string, rawError: unknown): string {
  if (isStructuredCodexError(rawError)) {
    ...
    if (rawError.code === "codex_activate_error") {
      return "MindFlow 无法完成 Codex 的启动。请先重试一次；如果仍然失败，把详细错误发出来再继续排查。";
    }
  }
  ...
  if (action.includes("Install")) {
    return formatCodexInstallError(message, normalized);
  }
  ...
  if (
    ...
    return "MindFlow 无法启动内置的 Codex 运行时。你可以稍后重试；如果持续失败，再考虑更新应用。";
  }
...
export function CodexPanel({ visible, workspacePath, renderMode = "native" }: Props) {
  ...
  const [error, setError] = useState<CodexErrorDisplay | null>(null);
  ...
  const reportCodexPanelError = (action: string, rawError: unknown, context?: Record<string, unknown>) => {
    const display = buildCodexErrorDisplay(action, rawError);
    setError(display);
    reportOperationError({
      source: "CodexPanel",
      action,
      error: rawError,
      userMessage: display.message,
      context,
    });
  };
  ...
  useEffect(() => {
    ...
    const syncHealth = async () => {
      ...
      setError({ message, detail: runtimeIssue.message });
      reportOperationError({
        source: "CodexPanel",
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      await fetch(`${host.origin}/mindflow/state`, {
        method: "POST",
  ...
  return (
    <div className="flex-1 h-full w-full flex flex-col overflow-hidden min-h-0">
        ...
        <div className="flex items-center gap-2">
          {status?.installed && needsUpdate && (
            <button
              ...
              disabled={busy}
              className="h-8 px-2 rounded-md border border-border/60 bg-muted/40 hover:bg-muted/70 text-xs flex items-center gap-1 disabled:opacity-50"
      ...
      {error && (
        <div className="px-3 py-2 text-xs text-destructive border-b border-border/60 bg-destructive/5">
          <div>{error.message}</div>
          {error.detail && error.detail !== error.message && (
            <div className="mt-1 text-[11px] text-destructive/80 break-all">
              详细信息：{error.detail}
            </div>
          )}
        </div>
      ...
      {needsInstall && (
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="max-w-sm w-full rounded-xl border border-border/60 bg-card/60 p-4 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={busy || !workspacePath}
                className="h-9 px-3 rounded-lg border border-border/60 bg-muted/40 hover:bg-muted/70 text-sm font-medium flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {busy ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}
                Import VSIX
              </button>
              <button
                ...
                className="h-9 px-3 rounded-lg border border-border/60 bg-muted/40 hover:bg-muted/70 text-sm font-medium flex items-center gap-2"
              >
