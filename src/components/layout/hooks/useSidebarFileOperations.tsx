...
import { FolderOpen, AppWindow, Star, StarOff } from "lucide-react";
...
export function useSidebarFileOperations() {
  ...
  const { activePane, openSecondaryFile } = useSplitStore();
        ...
        const updatedTabs = tabs.map((tab) => {
          if (tab.type === "file" || tab.type === "diagram") {
            ...
            if (nextPath !== tab.path) {
              const nextName =
                tab.type === "file"
                  ? nextPath.split(/[/\\]/).pop()?.replace(/\.md$/i, "") || tab.name
                  : tab.name;
              return {
                ...tab,
                path: nextPath,
                name: nextName,
                id: tab.type === "diagram" ? `__diagram_${nextPath}__` : nextPath,
              };
            }
          }
          return tab;
  ...
  const handleCreateSubmit = useCallback(async () => {
    ...
    try {
      if (await exists(fullPath)) {
        reportOperationError({
          ...
          action: creating.type === "folder" ? "Create folder" : "Create note",
    ...
    setCreating(null);
  }, [creating, createValue, openFile, refreshFileTree, t.file.createFailed, t.file.fileExists, t.file.folderExists, t.sidebar.newFolder, t.sidebar.newNote]);
  ...
  const getContextMenuItems = useCallback(
    ...
    [handleCopyPath, handleDelete, handleNewFile, handleNewFolder, handleShowInExplorer, handleStartRename, isFavorite, t.favorites.add, t.favorites.remove, toggleFavorite],
  );
  ...
  const handleSelect = useCallback(
    (entry: FileEntry) => {
      ...
      if (!entry.is_dir) {
        ...
        if (name.endsWith(".pdf")) {
          reportOperationError({
            source: "Sidebar.handleSelect",
            action: "Open PDF in AI-focused workspace",
            error: "PDF preview is disabled in this AI-focused build",
            level: "warning",
    ...
    [openFile, splitView, activePane, openSecondaryFile],
  );
