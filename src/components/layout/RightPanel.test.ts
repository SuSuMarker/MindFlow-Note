...
describe("RightPanel accent styling", () => {
  ...
  it("keeps the docked file panel focused on current-note assistance instead of the full agent shell", () => {
    expect(rightPanelSource).toContain("<NoteConversationList />");
    expect(rightPanelSource).toContain("<ChatPanel />");
    expect(rightPanelSource).not.toContain("<ConversationList />");
    expect(rightPanelSource).not.toContain("<AgentPanel />");
  });
});
