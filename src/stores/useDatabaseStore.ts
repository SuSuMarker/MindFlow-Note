...
import {
  ...
  getTitleFromPath,
  belongsToDatabase,
} from "@/services/markdown/frontmatter";
...
function localizeTemplate(
  ...
  const columns = template.columns?.map((col) => {
    const name =
      content.columns?.[col.id as keyof typeof content.columns] || col.name;
    const optionNames =
      content.options?.[col.id as keyof typeof content.options];
  ...
  const views = template.views?.map((view) => {
    const viewName =
      content.views?.[view.type as keyof typeof content.views] || view.name;
...
function resolveDatabaseNoteFolder(db: Database): string {
  const configuredFolder =
    typeof db.noteFolder === "string" ? db.noteFolder.trim() : "";
  const normalizedConfigured =
    normalizeFolderSegments(configuredFolder).join("/");
...
function findTitleColumn(
  ...
  return columns.find(
    (column) =>
      column.type === "text" &&
      titleColumnNames.includes(column.name.toLowerCase()),
  );
}
    ...
    (set, get) => ({
      ...
      loadDatabase: async (dbId: string) => {
        ...
        try {
          ...
          const dbDef = normalizeDatabaseDefinition(
            JSON.parse(content) as Database,
          );
      ...
      loadRowsFromNotes: async (dbId: string) => {
        ...
        try {
          ...
          const dbDef = normalizeDatabaseDefinition(
            JSON.parse(dbContent) as Database,
          );
        ...
        const allMdFiles = Array.from(
          new Set<string>([...mdFiles, ...extraMdFiles]),
        );
        ...
        for (const filePath of allMdFiles) {
          try {
            ...
            if (hasFrontmatter && belongsToDatabase(frontmatter, dbId)) {
              let rowId = frontmatter.noteId
                ? String(frontmatter.noteId).trim()
                : "";
              if (!rowId || seenRowIds.has(rowId)) {
                rowId = generateNoteId();
                const contentWithNoteId = updateFrontmatter(content, {
                  noteId: rowId,
                });
                try {
              ...
              if (titleColumn && isBlankCellValue(cells[titleColumn.id])) {
                const fallbackTitle =
                  (frontmatter.title as string) || getTitleFromPath(filePath);
      ...
      createDatabase: async (options: CreateDatabaseOptions) => {
        ...
        const rawTemplate = options.template
          ? DATABASE_TEMPLATES[options.template]
          : DATABASE_TEMPLATES.blank;
        ...
        const dbWithRows: DatabaseWithRows = {
          ...
          columns: template.columns?.map((col) => ({
            ...
            {
              id: generateId(),
              name: t.database.templateContent.blank.columns.title,
          ...
          views: template.views?.map((v) => ({
            ...
            {
              id: generateId(),
              name: t.database.templateContent.blank.views.table,
              type: "table" as const,
            },
          ],
          activeViewId: "",
      ...
      addRow: async (dbId: string, cells?: Record<string, CellValue>) => {
        try {
          ...
          // Auto-populate cells from active view filter rules so the new row
          // stays visible under the current filter (similar to Notion behavior).
          const activeView = db.views.find((v) => v.id === db.activeViewId);
          if (activeView?.filters) {
            for (const rule of activeView.filters.rules) {
              if (!("columnId" in rule)) continue;
              const fr = rule as import("@/types/database").FilterRule;
              if (fr.columnId in draftCells) continue;
              if (
                fr.operator === "equals" &&
                fr.value !== null &&
                fr.value !== undefined
              ) {
                draftCells[fr.columnId] = fr.value;
              } else if (fr.operator === "is_checked") {
                draftCells[fr.columnId] = true;
              }
            }
          }
          ...
          if (titleColumn) {
            ...
            titleValue = String(
              draftCells[titleColumn.id] || draftCells[titleColumn.name] || "",
            );
          }
          ...
          const noteName =
            notePath.split("/").pop()?.replace(".md", "") || safeFileName;
          ...
          for (const [columnId, value] of Object.entries(draftCells)) {
            ...
            if (
              !["db", "noteid", "title", "createdat", "updatedat"].includes(
...
`;
      ...
      duplicateRow: async (dbId: string, rowId: string) => {
        ...
        const noteDirectory = getDatabaseNoteDirectory(vaultPath, db);
        await ensureDirectoryRecursive(noteDirectory);
        const notePath = await ensureUniquePath(
          `${noteDirectory}/${safeFileName}`,
          ".md",
        );
      ...
      deleteView: (dbId: string, viewId: string) => {
        set((state) => {
          ...
          const activeViewId =
            db.activeViewId === viewId ? newViews[0].id : db.activeViewId;
