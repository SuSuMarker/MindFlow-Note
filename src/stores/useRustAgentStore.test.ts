...
describe('useRustAgentStore', () => {
  ...
  describe('Session Management', () => {
    ...
    it('should not delete the last remaining session', () => {
      const store = useRustAgentStore.getState();
      const onlySessionId = store.currentSessionId;

      act(() => {
        store.deleteSession(onlySessionId!);
      });

      const afterDelete = useRustAgentStore.getState();
      expect(afterDelete.sessions).toHaveLength(1);
      expect(afterDelete.currentSessionId).toBe(onlySessionId);
    });
  ...
  describe('startTask', () => {
    ...
    it('should rename a new session from the first user task', async () => {
      const store = useRustAgentStore.getState();

      await store.startTask('请帮我整理一下项目发布清单', { workspace_path: '/tmp' });

      const state = useRustAgentStore.getState();
      const currentSession = state.sessions.find((session) => session.id === state.currentSessionId);
      expect(currentSession?.title).toBe('项目发布清单');
    });
  });

  describe('persistence', () => {
    it('persists state under the mindflow-agent key', () => {
      const store = useRustAgentStore.getState();
      ...
      const raw = localStorage.getItem('mindflow-agent');
