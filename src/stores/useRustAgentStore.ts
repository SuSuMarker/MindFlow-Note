...
import { extractConversationTitleFromText } from "@/lib/conversationTitle";
...
const resolveVaultPath = (): string | null => {
  ...
  try {
    const raw = window.localStorage.getItem("mindflow-workspace");
    if (!raw) return null;
    ...
    (set, get) => ({
      ...
      startTask: async (task: string, context: TaskContext) => {
        ...
        set((state) => {
          ...
          const nextTitle = extractConversationTitleFromText(task, t.common.newConversation);
          return {
            ...(isBusy
              ? { error: null }
              : {
                  ...
                  streamingContent: "",
                  streamingReasoning: "",
                  streamingReasoningStatus: streamingThinkingEnabled ? "streaming" as const : "idle" as const,
                }),
            ...
            sessions: state.sessions.map((session) =>
              session.id === state.currentSessionId
                ? {
                    ...session,
                    title: session.title === t.common.newConversation ? nextTitle : session.title,
                    messages: nextMessages,
                    updatedAt: Date.now(),
                  }
                : session
                ...
                ? {
                    ...
                    title: s.title === t.common.newConversation && state.messages.length > 0
                      ? extractConversationTitleFromText(
                          state.messages.find(m => m.role === "user")?.rawContent
