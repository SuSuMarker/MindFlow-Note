...
vi.mock("@/lib/tauri", () => ({
  ...
  getWorkspacePluginDir: async () => '/tmp/workspace/.mindflow/plugins',
}));
...
const workspaceThemePlugin: PluginInfo = {
  ...
  source: "workspace",
  root_path: "/tmp/workspace/.mindflow/plugins/theme-oceanic",
  entry_path: "/tmp/workspace/.mindflow/plugins/theme-oceanic/index.js",
...
describe("usePluginStore", () => {
  ...
  it("uses per-ribbon-item defaults when user has no override", () => {
    ...
    expect(store.isRibbonItemEnabled("hello-mindflow", "hello-ribbon", false)).toBe(false);
  });
