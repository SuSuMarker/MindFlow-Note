...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if config.jwt_secret == "dev-secret-change-me" {
        if cfg!(debug_assertions) {
            tracing::warn!(
                "MINDFLOW_JWT_SECRET is using the default value; do not use this in production."
            );
        } else {
            return Err("MINDFLOW_JWT_SECRET must be set for production".into());
        }
    }
    ...
    let bind_addr = config.bind.parse().map_err(|_| "invalid MINDFLOW_BIND")?;
    ...
    let app = Router::new()
        ...
        .layer(trace_layer);
    tracing::info!("MindFlow Sync Server listening on {}", bind_addr);
