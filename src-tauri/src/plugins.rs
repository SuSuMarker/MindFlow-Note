...
fn plugin_roots(app: &AppHandle, workspace_path: Option<&str>) -> Vec<(String, PathBuf)> {
    ...
    if let Some(workspace) = workspace_path {
        let workspace_root = Path::new(workspace).join(".mindflow").join("plugins");
            ...
            if app_contents.file_name().and_then(|name| name.to_str()) == Some("Contents") {
                if let Some(app_bundle) = app_contents.parent() {
                    if let Some(install_dir) = app_bundle.parent() {
                        return Ok(install_dir.join("mindflow-plugins"));
                    }
                }
            }
...
fn ensure_writable_plugin_dir(dir: &Path) -> Result<(), String> {
    ...
    let probe = dir.join(".mindflow-plugin-write-probe");
...
"#,
  ...
  const unregister = api.commands.registerSlashCommand({
    ...
    prompt: "请用两句话问候我，并提到这是来自 MindFlow plugin 的问候。"
  });
  const unregisterCommand = api.commands.registerCommand({
    id: "open-hello-view",
    title: "Open Hello MindFlow view",
  ...
  const cleanupTheme = api.ui.setThemeVariables({
    "--mindflow-plugin-accent": "#0ea5e9"
  });
  const removeStatus = api.ui.registerStatusBarItem({
    id: "hello-status",
    text: "hello-mindflow ready",
    align: "right",
    run: () => api.ui.notify("hello-mindflow status clicked")
  });
  const removeSettings = api.ui.registerSettingSection({
    id: "hello-settings",
    title: "Hello MindFlow Settings",
  ...
  const cleanupStyle = api.ui.injectStyle(`
    :root {
      --plugin-hello-ring: color-mix(in srgb, var(--mindflow-plugin-accent) 40%, transparent);
    }
  `, "hello-mindflow");
  ...
  const removePanel = api.workspace.registerPanel({
    id: "hello-panel",
    title: "Hello Panel",
    html: "<p>This panel is registered by hello-mindflow.</p>"
  });
  ...
  const removeShellSlot = api.workspace.registerShellSlot({
    slotId: "app-top",
    order: 900,
    html: "<div>Hello slot from hello-mindflow</div>"
  });
  ...
  api.ui.notify("hello-mindflow loaded");
...
pub async fn plugin_scaffold_example(
    ...
    let example_dir = plugins_dir.join("hello-mindflow");
...
mod tests {
    ...
    fn test_root(name: &str) -> PathBuf {
        let ts = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        let path = std::env::temp_dir().join(format!("mindflow-plugin-tests-{}-{}", name, ts));
    ...
    fn merge_keeps_invalid_without_shadowing_valid_plugin() {
        ...
        write_plugin(
            &workspace_root,
            "hello-mindflow",
            r#"{"id":"hello-mindflow","name":"Bad","version":"bad","entry":"index.js"}"#,
            Some("module.exports = () => {};"),
        );
        write_plugin(
            &user_root,
            "hello-mindflow",
            r#"{"id":"hello-mindflow","name":"Good","version":"1.0.0","entry":"index.js"}"#,
            Some("module.exports = () => {};"),
        );
        ...
        let valid = merged
            .iter()
            .find(|p| p.source == "user" && p.id == "hello-mindflow")
