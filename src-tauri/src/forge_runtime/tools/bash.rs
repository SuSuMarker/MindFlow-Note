...
struct ShellEnv {
    ...
    path: Vec<PathBuf>,
    mindflow_git_bash_path: Option<PathBuf>,
...
impl ShellEnv {
    fn current() -> Self {
        ...
        Self {
            ...
            path,
            mindflow_git_bash_path: env::var_os("MINDFLOW_GIT_BASH_PATH").map(PathBuf::from),
            opencode_git_bash_path: env::var_os("OPENCODE_GIT_BASH_PATH").map(PathBuf::from),
            doc_tools_bin_path: env::var_os("MINDFLOW_DOC_TOOLS_BIN").map(PathBuf::from),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn windows_prefers_mindflow_git_bash_path() {
        let temp = TempDir::new().unwrap();
        ...
        let env = ShellEnv {
            ...
            path: Vec::new(),
            mindflow_git_bash_path: Some(bash.clone()),
            opencode_git_bash_path: None,
            doc_tools_bin_path: None,
        };
