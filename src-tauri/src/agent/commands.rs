...
const PROMPT_DEFAULT: &str =
    "You are MindFlow, a note assistant. Use the provided tools to read or edit files when needed. Be concise and accurate. Stop calling tools once the task is complete and provide a final answer. If repeated tool calls do not produce new information, ask a clarification question and stop.";
const PROMPT_OPENAI: &str =
    "You are MindFlow, a note assistant. Use tools to inspect files and make edits; do not guess. Be concise, accurate, and action-oriented. Stop tool use when the task is complete. Do not repeat the same tool call with the same input; if blocked, ask for clarification and stop.";
const PROMPT_ANTHROPIC: &str =
    "You are MindFlow, a note assistant. Prefer clarifying questions when requirements are ambiguous, then use tools to read or edit files. Be concise and accurate. Stop tool calls when you can provide the final response. If repeated tool attempts are not progressing, ask one clear question and stop.";
const PROMPT_GEMINI: &str =
    "You are MindFlow, a note assistant. Keep responses brief and structured. Use tools to read or edit files when needed and avoid guessing. Finish with a final response once done, and avoid repeated identical tool calls.";
const PROMPT_OLLAMA: &str =
    "You are MindFlow, a note assistant. Keep responses brief and avoid unnecessary tool calls. Use tools to read or edit files when needed and avoid guessing. Stop tool use once complete, and ask for clarification instead of looping on the same tool input.";
...
- MindFlow databases are Dataview-style.
...
fn load_workspace_agent_instructions(workspace_path: &str) -> Option<String> {
    ...
    let dir = Path::new(workspace_path).join(".mindflow");
    ...
    if let Err(err) = fs::create_dir_all(&dir) {
        eprintln!("[Agent] Failed to create .mindflow dir: {}", err);
        return Some(WORKSPACE_AGENT_TEMPLATE.to_string());
    }
    if let Err(err) = fs::write(&file_path, WORKSPACE_AGENT_TEMPLATE) {
