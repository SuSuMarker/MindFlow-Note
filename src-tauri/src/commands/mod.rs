...
pub async fn browser_webview_freeze(app: AppHandle, tab_id: String) -> Result<(), AppError> {
    ...
    if let Some(webview) = app.get_webview(&webview_id) {
        ...
        let freeze_js = r#"
            (function() {
                // 保存原始函数
                if (!window.__mindflow_frozen) {
                    window.__mindflow_frozen = true;
                    window.__mindflow_original_setInterval = window.setInterval;
                    window.__mindflow_original_setTimeout = window.setTimeout;
                    window.__mindflow_interval_ids = [];
                    window.__mindflow_timeout_ids = [];
                    
                    // 暂停所有媒体
                    document.querySelectorAll('video, audio').forEach(el => {
                        if (!el.paused) {
                            el.__mindflow_was_playing = true;
                            el.pause();
                        }
                    });
...
pub async fn browser_webview_unfreeze(app: AppHandle, tab_id: String) -> Result<(), AppError> {
    ...
    if let Some(webview) = app.get_webview(&webview_id) {
        ...
        let unfreeze_js = r#"
            (function() {
                if (window.__mindflow_frozen) {
                    window.__mindflow_frozen = false;
                    
                    // 恢复媒体播放
                    document.querySelectorAll('video, audio').forEach(el => {
                        if (el.__mindflow_was_playing) {
                            el.play().catch(() => {});
                            delete el.__mindflow_was_playing;
                        }
                    });
