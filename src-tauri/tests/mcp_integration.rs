...
use mindflow_lib as mindflow;
use std::collections::HashMap;
use tempfile::TempDir;

/// 创建测试用的 MCP 配置
fn create_test_config(temp_dir: &TempDir) -> String {
    let config_dir = temp_dir.path().join(".mindflow/settings");
...
async fn test_user_scenario_no_mcp_config() {
    ...
    // 这里我们直接测试配置加载
    let config = mindflow::mcp::config::load_mcp_config(workspace_path).await;
...
async fn test_user_scenario_disabled_server() {
    ...
    let workspace_path = temp_dir.path().to_str().unwrap();
    let loaded = mindflow::mcp::config::load_mcp_config(workspace_path)
        .await
        .unwrap();
...
async fn test_user_scenario_save_and_reload_config() {
    ...
    let config = mindflow::mcp::McpConfig {
        mcp_servers: servers,
    };

    // 保存配置
    mindflow::mcp::config::save_mcp_config(workspace_path, &config)
        .await
        .unwrap();
