module.exports = function setup(api) {
  ...
  const inspectWorkspace = async ({ force = false } = {}) => {
    ...
    const bridgeNotes = normalized.filter((path) =>
      path.startsWith(".mindflow/openclaw-bridge-") && path.toLowerCase().endsWith(".md"),
    );
  ...
  const buildBridgePath = (kind) =>
    `.mindflow/openclaw-bridge-${kind}-${new Date().toISOString().replace(/[:.]/g, "-")}.md`;
  ...
  const stageBridgeNote = async (kind, content, metadata) => {
    return await errorReporter.wrapOperation(
      ...
      async () => {
        ...
        const body = [
          "---",
          "source: mindflow-openclaw-bridge",
          `kind: ${kind}`,
  ...
  const renderOverview = (snapshot, cronJobs) => {
    ...
    if (!snapshot.workspacePath) {
      return [
        ...
        "<p>Open any MindFlow workspace, then attach an OpenClaw workspace path to use this integration.</p>",
      ].join("");
    }
    ...
    const guidance = snapshot.attached
      ...
      : "<p>Choose an OpenClaw workspace path in settings, then use <code>Attach OpenClaw workspace</code> to mount it into the current MindFlow workspace.</p>";
    ...
    return [
      ...
      bridgeItems ? `<ul>${bridgeItems}</ul>` : "<p>No MindFlow bridge notes have been staged yet.</p>",
      ...
      async () => {
        if (!api.workspace.getPath()) {
          errorReporter.report({
            ...
            level: 'warning',
            userMessage: t('openClawNoWorkspace'),
            detail: 'Attempted to attach OpenClaw workspace without opening a MindFlow workspace',
            error: new Error('No workspace open'),
        ...
        commands: [
          {
            ...
            description: snapshot.attached
              ...
              : "Attach an external OpenClaw workspace to the current MindFlow workspace.",
            run: () => {
          ...
          },
          {
            ...
            description: "Clear the mounted OpenClaw workspace state for this MindFlow workspace.",
            run: () => {
          ...
          },
          {
            ...
            description: "Write the current note into a MindFlow bridge note inside the workspace.",
            run: () => {
          ...
          },
          {
            ...
            description: "Write the current editor selection into a MindFlow bridge note inside the workspace.",
            run: () => {
              void stageSelection();
            },
          },
